...
// Define file sources.
var sassFrontendSource        = [ 'dev/scss/wpbooklist-categories-main-frontend.scss' ];
var sassFrontendSourcePartial = [ 'dev/scss/_wpbooklist-categories-frontend-ui.scss' ];
var sassBackendSource         = [ 'dev/scss/wpbooklist-categories-main-admin.scss' ];
var sassBackendSourcePartial  = [ 'dev/scss/_wpbooklist-categories-backend-ui.scss' ];
...
gulp.task( 'concatAdminJs', function() {
	gulp.src(jsBackendSource ) // use jsSources
		.pipe(concat( 'wpbooklist_categories_admin.min.js' ) ) // Concat to a file named 'script.js'
...
gulp.task( 'concatFrontendJs', function() {
	gulp.src(jsFrontendSource ) // use jsSources
		.pipe(concat( 'wpbooklist_categories_frontend.min.js' ) ) // Concat to a file named 'script.js'
		.pipe(uglify() ) // Uglify concatenated file
		.pipe(gulp.dest( 'assets/js' ) ); // The destination for the concatenated and uglified file
});

gulp.task( 'copyassets', function () {
	gulp.src([ './assets/**/*' ], {base: './'}).pipe(gulp.dest( '../wpbooklist-categories_dist' ) );
});

gulp.task( 'copyincludes', function () {
	gulp.src([ './includes/**/*' ], {base: './'}).pipe(gulp.dest( '../wpbooklist-categories_dist' ) );
});

gulp.task( 'copyconfig', function () {
	gulp.src([ './wpbooklistconfig.ini' ], {base: './'}).pipe(gulp.dest( '../wpbooklist-categories_dist' ) );
});

gulp.task( 'copyreadme', function () {
	gulp.src([ './readme.txt' ], {base: './'}).pipe(gulp.dest( '../wpbooklist-categories_dist' ) );
});

gulp.task( 'copylang', function () {
	gulp.src([ './languages/**/*' ], {base: './'}).pipe(gulp.dest( '../wpbooklist-categories_dist' ) );
});

gulp.task( 'copymainfile', function () {
	gulp.src([ './wpbooklist-categories.php' ], {base: './'}).pipe(gulp.dest( '../wpbooklist-categories_dist' ) );
});

gulp.task('copyuifile', function () {
    gulp.src(['./class-admin-display-categories-tab-extension-ui.php'], {base: './'}).pipe(gulp.dest('../wpbooklist-categories_dist'));
});

gulp.task( 'zip', function () {
	return gulp.src( '../wpbooklist-categories_dist/**' )
		.pipe(zip( 'wpbooklist-categories.zip' ) )
		.pipe(gulp.dest( '../wpbooklist-categories_dist' ) );
});

gulp.task( 'cleanzip', function(cb) {
	del([ '../wpbooklist-categories_dist/**/*' ], {force: true}, cb);
});

gulp.task( 'clean', function(cb) {
	del([ '../wpbooklist-categories_dist/**/*', '!../wpbooklist-categories_dist/wpbooklist-categories.zip' ], {force: true}, cb);
});
